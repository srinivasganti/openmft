defmodule Openmft.Ui.Dsl.Verifiers.DataTable.AllColumnsValid do
  @moduledoc """
  Ensures all columns in each action are valid fields or relationship paths.
  """
  ...
  def verify(dsl) do
    ...
    for %Action{} = action <- Verifier.get_entities(dsl, [:data_table]) do
      for %Column{} = column <- action.columns |> Map.values() do
        ...
        source_resource = Openmft.Ui.Info.resource_by_path(resource, path)
